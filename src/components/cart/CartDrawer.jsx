...
const CartDrawer = ({ isOpen, onClose }) => {
  ...
  /**
   * Cart state
   * - Fallback to `{}` if state.cart is undefined (prevents runtime crashes)
   * - Destructure with defaults so UI never breaks when slice is missing or empty
   */
  const {
    items = [],
    totalAmount = 0,
   ...
   * Listens on document and checks if click target is outside the drawer ref.
   */
  useEffect(() => {
   ...
   * Prevents background content from scrolling under the overlay.
   */
  useEffect(() => {
  ...
  // Avoid rendering anything if the drawer is closed
  ...
  return (
    <div className="fixed inset-0 z-60 flex justify-end bg-black/50 backdrop-blur-sm animate-in fade-in duration-300">
      ...
      >
        ...
        {/* -------------------------------------------------- */}
        {/* BODY: LIST OF CART ITEMS / EMPTY STATE             */}
        {/* -------------------------------------------------- */}
              ...
              >
                ...
                <div className="h-28 w-20 shrink-0 overflow-hidden rounded-md bg-gray-100 border border-gray-200">
                  <img
                    ...
                    onError={(e) => {
                      // Basic fail-safe: hide broken image
